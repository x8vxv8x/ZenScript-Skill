...
| 方法 | 返回 | 说明 |
|------|------|------|
| `.addRecipe(IIngredient coreInput, IIngredient casingInput, IAmmunitionWorkshopFunction fun, int energy, int time)` | void | 添加弹药工坊配方。`fun` 为输出函数，接收 (IItemStack 核心输入, IItemStack 弹壳输入) 返回 IItemStack 输出。不限于弹药，可用于任意双输入配方 |
| `.removeRecipe(IItemStack input)` | void | 按核心输入移除配方 |

#### IAmmunitionWorkshopFunction

输出函数类型，用于根据两个输入物品决定输出物品（如获取核心 NBT 标签或合并核心和弹壳 NBT 标签）。

- 参数：`(IItemStack 核心输入, IItemStack 弹壳输入)`
- 返回：`IItemStack`
