...
| 方法 | 参数 | 返回 | 说明 |
|------|------|------|------|
| `.update(IData)` | IData | void | 更新玩家数据 |
| `.give(IItemStack)` | IItemStack | void | 给予物品 |
| `.sendChat(string)` | string | void | 发送消息 |
| `.sendChat(IChatMessage)` | IChatMessage | void | 发送富文本消息 |
| `.sendStatusMessage(string, @Optional bool)` | string, bool（可选，默认 true） | void | 发送状态消息。第二个参数为 true 时显示在快捷栏 |
| `.sendRichTextStatusMessage(ITextComponent, @Optional bool)` | ITextComponent, bool（可选） | void | 发送富文本状态消息 |
| `.sendToast(string, string)` | string, string | void | 发送通知 |
| `.executeCommand(string)` | string | void | 执行命令 |
| `.attackEntityFrom(IDamageSource, float)` | IDamageSource, float | void | 造成伤害 |
| `.heal(float)` | float | void | 治疗 |
| `.setHealth(float)` | float | void | 设置生命值 |
| `.setFoodLevel(int)` | int | void | 设置饥饿值 |
| `.setSaturation(float)` | float | void | 设置饱和度 |
| `.addExperience(int)` | int | void | 添加经验 |
| `.removeXP(int)` | int | void | 移除经验等级 |
| `.setXPLevel(int)` | int | void | 设置经验等级 |
| `.getHeldItem(EnumHand)` | EnumHand | IItemStack | 获取手持物品 |
| `.setItemToSlot(EntityEquipmentSlot, IItemStack)` | EntityEquipmentSlot, IItemStack | void | 设置装备 |
| `.hasGameRule(string)` | string | bool | 是否有游戏规则 |
| `.getGameRule(string)` | string | string | 获取游戏规则 |
| `.getHotbarStack(int)` | int | IItemStack | 获取快捷栏指定位置的物品 |
| `.getInventoryStack(int)` | int | IItemStack | 获取背包指定位置的物品 |
| `.teleport(Position3f)` | Position3f | void | 传送到指定位置（同一维度） |
| `.dropItem(bool)` | bool | void | 丢弃当前手持物品。true 丢弃整个堆叠 |
| `.dropItem(IItemStack)` | IItemStack | void | 在玩家位置丢弃指定物品 |
| `.getCooldown(IItemStack)` | IItemStack | int | 获取指定物品的冷却时间 |
| `.setCooldown(IItemStack, int)` | IItemStack, int | void | 设置指定物品的冷却时间 |
...
| 方法 | 参数 | 返回 | 说明 |
|------|------|------|------|
| `.addStats(int, float)` | int, float | void | 添加食物值和饱和度 |
| `.onUpdate(IPlayer)` | IPlayer | void | 更新食物状态 |
| `.asNBT()` | 无 | IData | 获取食物状态的 NBT 数据 |
| `.addExhaustion(float)` | float | void | 添加疲劳值 |
